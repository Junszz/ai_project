...
def train(args, model, dataloader, loader_len, criterion, optimizer, scheduler, use_gpu, epoch, save_file_name='train.csv'):
    ...
    if epoch == 0:
        resultWriter.create_csv(['epoch', 'loss', 'accuracy', 'lr'])
    ...
    resultWriter.write_csv([epoch, losses.avg, accuracy.avg.item(), scheduler.optimizer.param_groups[0]['lr']])
    # print('[Train #{}] Loss: {:.4f} Acc: {:.4f}% Time: {:.4f}s'.format(epoch, epoch_loss, epoch_acc, time.time() - start_time))
    print('lr:%.6f' % scheduler.optimizer.param_groups[0]['lr'])
    print('Train ***    Loss:{losses.avg:.2e}    Acc:{accuracy.avg:.2f} '.format(losses=losses, accuracy=accuracy))
...
def validate(args, model, dataloader, loader_len, criterion, use_gpu, epoch, ema=None, save_file_name='val.csv'):
    ...
    resultWriter.write_csv([epoch, losses.avg, accuracy.avg.item()])

    print(' Val  ***    Loss:{losses.avg:.2e}    Acc@1:{accuracy.avg:.2f} '.format(losses=losses, accuracy=accuracy))
...
def train_model(args, model, dataloader, loaders_len, criterion, optimizer, scheduler, use_gpu):
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        accuracy  = validate(args, model, dataloader['val'], loaders_len['val'], criterion, use_gpu, epoch)
...
if __name__ == "__main__":
    ...
    def train(epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            print('Train ***    Loss:{losses:.2e}    Acc:{accuracy:.2f} '.format(losses=losses, accuracy=accuracy))
    ...
    def test(epoch):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(valid_loader):
                ...
                accuracy = 100.*correct/total
                print('Test ***    Loss:{losses:.2e}    Acc:{accuracy:.2f} '.format(losses=losses, accuracy=accuracy))
